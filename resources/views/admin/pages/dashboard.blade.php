...
@section('content')
    ...
    <div id="quickReservationModal" class="modal fade unselectable" tabindex="-1" role="dialog"
         aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel">Masa rezerv et</h4>
                    <button type="button" class="btn btn-sm btn-outline-dark" data-dismiss="modal" aria-hidden="true">
                        Bağla
                    </button>
                </div>
                <div class="modal-body">
                    <div class="mb-2 mr-sm-2 input-group date col-xs-4" id="datetimepickerInfo"
                         data-target-input="nearest">
                        <input type="text" name="res_date_info"
                               value="{{\Carbon\Carbon::now('Asia/Baku')->toDateTimeString()}}"
                               class="form-control datetimepicker-input" data-target="#datetimepickerInfo"/>
...
@section('js')
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        let bookTableBtn = $(`<button class="btn btn-sm btn-secondary waves-effect waves-light book-table" type="button"
                                              ...
                                              >
                                    <span class="btn-label"><i class="fas fa-check"></i></span> Book
                                  </button><br>`)
        ...
        $(document).on('click', '.book-table', function () {
            let _this = $(this)
            let table_id = $(this).data('table-id')
            let table_status = $(this).data('table-booked')
            ...
            if(table_id){
                $.ajax({
                    ...
                    success: (response) => {
                        $(this).data('table-booked', response.table.status)
                        let currentTable = $(`.tableDiv[data-table-id="${table_id}"]`);
                        let currentColor = currentTable.data('table-res-amount') ? 'green' : 'grey';
                        if(parseInt(response.table.status) === 1){
                            currentTable.addClass('bg-danger')
                            currentTable.css('background-color', '')
                        }else{
                            currentTable.removeClass('bg-danger')
