    ...
    $.fn.imageMaps = function(options) {
        ...
        this.each(function(i, item){
            ...
            $(options.addBtn+':eq('+ i +')').unbind('click').on('click', function(){
                ...
                let lastIndex = _this.children('div').length ? _this.children('div').eq(count-1).data('index')+1 : 0
                ...
                let oTr    = $(itemTemplate.replace(/###/g, lastIndex+1));
                output.append(oTr);
                ...
                if(doingRecover==false){
                    oMap.append(`<area shape="rect" name="imagemaps-area" class="imagemaps-area${lastIndex}" coords="0,0,${options.rectWidth},${options.rectHeight}" href="" target="_self"/>`);
                }
                ...
                oTr.find(options.areaHref+','+options.areaTarget).on('change', function(){
                    active       = $(`#imagemaps-rect-${i}-${lastIndex}`);
                    let oWrapper = active.parent();
                    let oArea    = oWrapper.find('.imagemaps .imagemaps-area'+lastIndex);
                    let output   = $(options.output).eq(i);

                    oArea.attr('href', output.find(`.item-${lastIndex+1} `+options.areaHref).val());
                    oArea.attr('target', output.find(`.item-${lastIndex+1} `+options.areaTarget).val());
                });

                // 删除热区
                oTr.find(options.btnDelete).unbind('click').on('click', function(){
                    // let dell = $(this)
                    // console.log(dell.parent().parent())
                    active       = $(`#imagemaps-rect-${i}-${lastIndex}`);
                    let oWrapper = active.parent();
                    let oArea    = oWrapper.find('.imagemaps .imagemaps-area'+lastIndex);
                    let output   = $(options.output).eq(i);
                    ...
                    active.remove();
                    oArea.remove();
                    output.find(`.item-${lastIndex+1}`).remove();
